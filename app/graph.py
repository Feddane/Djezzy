...
def bubble(property="famille", month=None, categorie=None):
    ...
    if month:
        query = query.filter(extract('year', Reclamation.date_fin) == extract('year', func.current_date()))
        query = query.filter(extract('month', Reclamation.date_fin) == month)

    if categorie:
        query = query.filter(Reclamation.categorie == categorie)

    query = query.group_by(getattr(Reclamation, property))
...
def horizentalBar(month=None, categorie=None):
    ...
    current_year = func.extract('year', func.current_date())
    query = db.session.query(Reclamation.operateur, func.count().label('count'))
    ...
    query = query.group_by(Reclamation.operateur)
...
def verticalBar(month=None):
    ...
    current_year = func.extract('year', func.current_date())
    query = db.session.query(Reclamation.categorie, func.count().label('count'))

    if month:
        query = query.filter(extract('year', Reclamation.date_fin) == extract('year', func.current_date()))
        query = query.filter(extract('month', Reclamation.date_fin) == month)

    query = query.group_by(Reclamation.categorie)
...
def plotmois(categorie=None):
    query = db.session.query(func.to_char(Reclamation.date_ouverture, "YYYY-MM-DD"))
    query = query.filter(Reclamation.date_ouverture.is_not(None))
    query = query.filter(extract('year', Reclamation.date_fin) == extract('year', func.current_date()))
